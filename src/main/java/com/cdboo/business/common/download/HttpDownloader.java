    ...
    public void run() {
        ...
        if (info.isNeedDownload(chp)) {
            ...
            try {
                ...
                Config.getConfigInstance().setMessage(null);
                info.deleteFlagFile();
        ...
        public void run() {
            ...
            while (curRetry < maxRetry && !isDone) {
                try {
                    ...
                    logger.info(__info.getPair().localName + " #Block" + (curIndex + 1) + "# begin downloading...");
                    int length;
                    long counter = 0;
                    ...
                    while (!isDone && startPos < endPos && (length = is.read(buffer)) > 0) {
                        ...
                        if (counter % 20 == 0) {
                            ...
                            Config.getConfigInstance().setMessage(__info.getPair().localName + "下载进度" + getPercentage());
                            Thread.yield();
                        }
                    }
                    __info.writeInfo(chp);
